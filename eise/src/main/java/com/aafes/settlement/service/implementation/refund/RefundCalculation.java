	...
	BaseCalculation
{
	...
	private static final Logger LOGGER = LogManager
			.getLogger(RefundCalculation.class);
	...
	)
	{
		...
		LOGGER.debug("---- Refund Request: calculatePaymentTenders(): END---");
	}
	...
	{
		LOGGER.info("---- Refund Request: settlementBusinessLogic(): START---");
		LOGGER.debug(
				"The rule for refund is Lowest GC followed by highest and lastly Credit Card and In case of Milstar Card attached, after reserving amount for pending shipment/refund items for specific line (Retail or Uniform)"
		);
		super.settlementBusinessLogic(
				p_calculationContainer,
				p_settlementContainer, p_repoContainer, p_eiseResponseGeneric
		);
		LOGGER.info("---- Refund Request: settlementBusinessLogic(): END---");
	}
