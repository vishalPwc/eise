...
@Service
public class RetrunsSettlementService
	implements Constants
{
	 ...
	 */
	public void processOrderlineRetruns(
		ReturnRequest 	   p_returnRequest
	,	ResponseSettlement p_responseSettlement
	) {
		
		ResponseTender l_refundTender 	  = new ResponseTender();
		List<ResponseTender> 
					   l_refundTenderList = new ArrayList<ResponseTender>();
		try {
			List<OpenInvoice> l_openInvoiceList = new ArrayList<OpenInvoice>();
			l_openInvoiceList = p_returnRequest.getOpenInvoice();
			...
			for (OpenInvoice openInvoice : l_openInvoiceList) {
				...
				processInvoiceLines(
						...
						l_refundTender, p_returnRequest

				);
			}

			l_refundTenderList.add(l_refundTender);
			p_responseSettlement.setRefundTender(l_refundTenderList);
			...
			throws Exception
	{
		...
		for (InvoiceLine invoiceLine : p_invoiceLineList) {
			l_authInterface = getTenderImpl(invoiceLine, p_returnRequest);

			if (p_invoiceType.equals(INVOICE_TYPE_R)) {
				l_authInterface.processReturns(
						invoiceLine,
						l_refundTender, p_returnRequest, p_openInvoice
